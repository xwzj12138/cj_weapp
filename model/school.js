...
export default new class school extends auth{
  ...
  search(param, callback) {
    this.request({ url: 'index/v1/school/search', type: 'POST', data: param, sCallBack: (res) => { callback(res); } });
  }
  //在某个门派中发布主题
  publishTheme(param, callback) {
    this.request({ url: 'index/v1/school/publishTheme', type: 'POST', data: param, sCallBack: (res) => { callback(res); } });
  }
  //在某个门派中的主题列表
  getTheme(param, callback) {
    this.request({ url: 'index/v1/school/getTheme', type: 'POST', data: param, sCallBack: (res) => { callback(res); } });
  }
  //获取某个主题详情
  themeDetail(param, callback) {
    this.request({ url: 'index/v1/school/themeDetail', type: 'POST', data: param, sCallBack: (res) => { callback(res); } });
  }
  //评论主题
  commentTheme(param, callback) {
    this.request({ url: 'index/v1/school/commentTheme', type: 'POST', data: param, sCallBack: (res) => { callback(res); } });
  }
  //获取主题的评论列表
